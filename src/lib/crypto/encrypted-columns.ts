 ...
 *   - ct as-is if ct is non-null but dek is null (shows as "v1:..." so the
 *     user knows they need to re-auth; same pattern as transaction payees)
...
export function decryptName(
  ...
  if (ct != null && ct !== "") {
    if (dek) return decryptField(dek, ct);
