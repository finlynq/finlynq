...
export function registerV2Tools(server: McpServer, sqlite: Database.Database) {
  // ---- READ TOOLS ----
  ...
  server.tool(
    ...
    async () => {
      ...
      const incomeExpenses = sqlite.prepare(
      ...
      const budgetsData = sqlite.prepare(
  ...
  server.tool(
    ...
    async ({ days }) => {
      ...
      const bankAccounts = sqlite.prepare(
  ...
  server.tool(
    ...
    async ({ payee, min_amount, max_amount, start_date, end_date, category, tags, limit }) => {
      ...
      const rows = sqlite.prepare(query).all(...params) as TransactionRow[];
  ...
  server.tool(
    ...
    async ({ months, currency }) => {
      ...
      const rows = sqlite.prepare(query).all(...params) as NetWorthRow[];
      ...
      const baselines = sqlite.prepare(baselineQuery).all(...baseParams) as { currency: string; total: number }[];
  ...
  server.tool(
    ...
    async ({ transaction_id, category }) => {
      const cat = sqlite.prepare("SELECT id, name FROM categories WHERE name = ?").get(category) as { id: number; name: string } | undefined;
  ...
  server.tool(
    ...
    async ({ name, type, group, currency, note }) => {
      ...
      const existing = sqlite.prepare("SELECT id FROM accounts WHERE name = ?").get(name) as { id: number } | undefined;
