...
`(user_id, account_id, name)` already exists.
...
### `tx.portfolio_holding` = holding NAME, not symbol

The aggregator joins on `portfolio_holdings.name`. Ticker lives in
`portfolio_holdings.symbol` for the price service. Writing the symbol on
the transaction makes every holding display `Qty: --`.
...
2. **`tx.portfolio_holding` = holding NAME, not ticker symbol.** See §6.
